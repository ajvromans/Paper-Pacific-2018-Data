function timeend  = NewGypsum4(phii10,phii30,del,epsilon,KK,phirct,JJ,phires,AA,gamma,simulation)
%--------------------------------------------------------------------------
...
kappa1 = 23.00*10^(KK);
kappa3 = 13.50*10^(KK);
gamma1 = 10000*10^(0.5*(gamma+4));
gamma2 = 10000*10^(0.5*(gamma+4));
...
tol = 0.1; %The value of phimin. Basically the resolution of 0.
...
if (min([phi10,phi30,1-phi10-phi30]) < tol)
    ...
    tolerance = (1-tol)*tanh(1);
    if (max([phi10+phi30,phi10+phi20,phi20+phi30])>tolerance)
        disp('Initial concentration outside Steinmetz Solid. Program continues.');
        disp(phi10+phi30>tolerance);
        disp(phi10+phi20>tolerance);
        disp(phi20+phi30>tolerance);
    end
end
...
function dydx = odev(x,y)
    dydx = [y(3);
        y(4);
        chi1*(1-phi20)/(gamma1*phi30)*y(1)+chi1*phi20/(gamma1*phi30)*y(2)-chi1*FF*x/gamma1-chi1*CC/gamma1;
        chi2*(1-phi10)/(gamma2*phi30)*y(2)+chi2*phi10/(gamma2*phi30)*y(1)-chi2*FF*x/gamma2-chi2*CC/gamma2];
end 
...
function dydx = odew(x,y)
    ...
    B1  = chi1/(gamma1+Ehat1*DT);
    B2  = chi2/(gamma2+Ehat2*DT);
    C11 = Ehat1*DT/(gamma1+Ehat1*DT);
    C12 = Ehat2*DT/(gamma1+Ehat1*DT);
    C21 = Ehat1*DT/(gamma2+Ehat2*DT);
    C22 = Ehat2*DT/(gamma2+Ehat2*DT);
    ...
    dydx = [y(3);
         y(4);
         ypstar(8,1)+B1*(y(1)-ystar(4,1)-DT*ystar(3,1))-C11*(ystar(6,1)*...
            ypstar(4,1)+ystar(1,1)*ypstar(8,1))-C12*(ystar(6,1)*ypstar(5,1)+...
            ystar(1,1)*ypstar(9,1));
         ypstar(9,1)+B2*(y(2)-ystar(5,1)-DT*ystar(3,1))-C21*(-(ystar(6,1)+...
            ystar(7,1))*ypstar(4,1)+(1-ystar(1,1)-ystar(2,1))*ypstar(8,1))-...
            C22*(-(ystar(6,1)+ystar(7,1))*ypstar(5,1)+(1-ystar(1,1)-...
            ystar(2,1))*ypstar(9,1))];
end 
...
for t = 1:M
    ...
    if (min([min(Z1(1+t,:)),min(Z2(1+t,:)),min(Z3(1+t,:))]) < tol)
        ...
        filedirectory = 'D:\Simulations\Existence-simulations\Standard\'; %specific directory
